    ...
    <div class="space-y-6">
      ...
      <ui-panel heading="The interrupt controller">
        <p class="mb-3 text-sm text-slate-700">
          ...
          <code>on_interrupt</code> custom event). The LlamaIndex agent in
          this repo runs the stock workflow from
          <code>get_ag_ui_workflow_router</code>, which never emits one, so this
          half stays idle. The controller clears
